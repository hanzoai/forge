...
import (
	...
	lfs_module "github.com/hanzoai/git/modules/lfs"
...
func DownloadHandler(ctx *context.Context) {
	...
	ctx.Resp.Header().Set("X-LFS-Content-Length", contentLengthStr) // we need this header to make sure it won't be affected by reverse proxy or compression
...
func requireAuth(ctx *context.Context) {
	ctx.Resp.Header().Set("WWW-Authenticate", `Basic realm="hanzo-git-lfs"`)
