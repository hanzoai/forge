...
func addDefaultHeaders(req *http.Request, secret []byte, w *webhook_model.Webhook, t *webhook_model.HookTask, payloadContent []byte) error {
	...
	// X-Git-* is ours and is what our own consumers read. The Gogs and GitHub
	// families below carry the same values so a receiver written for one of those
	// servers works against us unchanged — that is what this block is for.
