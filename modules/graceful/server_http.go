...
func newHTTPServer(network, address, name string, handler http.Handler) (*Server, ServeFunction) {
	...
	protocols.SetHTTP2(true)            // HTTP/2 can only be used when the forge is configured to use TLS
	protocols.SetUnencryptedHTTP2(true) // Allow HTTP/2 without TLS, in case the forge is behind a reverse proxy
