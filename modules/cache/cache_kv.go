...
// KVCacher represents a KV cache adapter implementation.
...
// toStr convert string/int/int64 interface to string. it's only used by the KVCacher.Put internally
...
func (c *KVCacher) Put(key string, val any, expire int64) error {
...
func (c *KVCacher) Get(key string) any {
...
func (c *KVCacher) Delete(key string) error {
	key = c.prefix + key
...
func (c *KVCacher) Incr(key string) error {
	if !c.IsExist(key) {
...
func (c *KVCacher) Decr(key string) error {
	if !c.IsExist(key) {
		return fmt.Errorf("key '%s' not exist", key)
	}
...
func (c *KVCacher) IsExist(key string) bool {
...
func (c *KVCacher) StartAndGC(opts cache.Options) error {
	c.hsetName = "MacaronCache"
	c.occupyMode = opts.OccupyMode

	uri := nosql.ToKVURI(opts.AdapterConfig)

	c.c = nosql.GetManager().GetKVClient(uri.String())
