...
var (
	...
	AppWorkPath string
	CustomPath  string // Custom directory path. Env: GIT_CUSTOM
	CustomConf  string
...
func InitWorkPathAndCfgProvider(getEnvFn func(name string) string, args ArgWorkPathAndCustomConf) {
	...
	readFromEnv := func() {
		envWorkPath := getEnvFn("GIT_WORK_DIR")
		if envWorkPath != "" {
			tmpWorkPath.Set(envWorkPath)
			if !filepath.IsAbs(tmpWorkPath.Value) {
				log.Fatal("GIT_WORK_DIR (work path) must be absolute path")
			}
		}

		envCustomPath := getEnvFn("GIT_CUSTOM")
		if envCustomPath != "" {
			tmpCustomPath.Set(envCustomPath)
			if !filepath.IsAbs(tmpCustomPath.Value) {
				log.Fatal("GIT_CUSTOM (custom path) must be absolute path")
			}
		}
	}
	...
	if configWorkPath != "" {
		...
		if tmpWorkPath.Value != "" && (getEnvFn("GIT_WORK_DIR") != "" || args.WorkPath != "") {
