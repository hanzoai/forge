...
test('updateWorkflowBadgeFields updates badge snippets for selected branch', () => {
  document.body.innerHTML = `
    <div
      data-badge-url="https://forge.example.com/user1/repo1/actions/workflows/build/test%20workflow.yml/badge.svg?branch=main"
      data-workflow-url="https://forge.example.com/user1/repo1/actions?workflow=build%2Ftest+workflow.yml"
  ...
  const badgeURL = 'https://forge.example.com/user1/repo1/actions/workflows/build/test%20workflow.yml/badge.svg?branch=release%2F1.0+%26+hotfix';
  ...
  expect(form.querySelector<HTMLTextAreaElement>('#workflow-badge-markdown')!.value).toBe(
    `[![CI \\[prod\\]\\\\build "fast" <ok>](${badgeURL})](https://forge.example.com/user1/repo1/actions?workflow=build%2Ftest+workflow.yml)`,
  );
  expect(form.querySelector<HTMLTextAreaElement>('#workflow-badge-html')!.value).toBe(
    `<a href="https://forge.example.com/user1/repo1/actions?workflow=build%2Ftest+workflow.yml"><img src="${badgeURL}" alt="CI [prod]\\build &quot;fast&quot; &lt;ok&gt;"></a>`,
  );
});
