...
// AuthorizedStringCommentPrefix marks the lines in authorized_keys this instance
// manages, so RegeneratePublicKeys replaces them and leaves keys the user added
// by hand alone.
const AuthorizedStringCommentPrefix = `# forge public key`

// legacyAuthorizedStringCommentPrefixes are the markers earlier releases wrote.
// Recognising them is what makes a revoked key stop working: a marker we do not
// know is copied through as a hand-added key, and the key beneath it stays
// authorized for as long as the file lives.
var legacyAuthorizedStringCommentPrefixes = []string{`# gitea public key`}

// IsManagedAuthorizedLine reports whether a line in authorized_keys is a marker
// this instance wrote. The key itself is on the following line.
func IsManagedAuthorizedLine(line string) bool {
	if strings.HasPrefix(line, AuthorizedStringCommentPrefix) {
		return true
	}
	for _, prefix := range legacyAuthorizedStringCommentPrefixes {
...
func RegeneratePublicKeys(ctx context.Context, t io.Writer) error {
	...
	if isExist {
		...
		for scanner.Scan() {
			line := scanner.Text()
			if IsManagedAuthorizedLine(line) {
				scanner.Scan()
				continue
			}
