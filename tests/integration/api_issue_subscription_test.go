...
func TestAPIIssueSubscriptions(t *testing.T) {
	...
	testSubscription := func(issue *issues_model.Issue, isWatching bool) {
		...
		req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/subscriptions/check", issueRepo.OwnerName, issueRepo.Name, issue.Index)).
			AddTokenAuth(token)
	...
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/%d/subscriptions/%s", issue1Repo.OwnerName, issue1Repo.Name, issue1.Index, owner.Name)
	...
	urlStr = fmt.Sprintf("/v1/repos/%s/%s/issues/%d/subscriptions/%s", issue5Repo.OwnerName, issue5Repo.Name, issue5.Index, owner.Name)
