	...
	onGitRun(t, func(t *testing.T, u *url.URL) {
		...
		for _, branch := range []string{"master", ""} {
			t.Run("Branch-"+branch, func(t *testing.T) {
				...
				req := NewRequestWithJSON(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/contents", user2.Name, repo1.Name), &changeFilesOptions).
					AddTokenAuth(token2)
		...
		url := fmt.Sprintf("/v1/repos/%s/%s/contents", user2.Name, repo1.Name)
		...
		protectionReq := NewRequestWithJSON(t, "POST", "/v1/repos/user2/repo1/branch_protections", &api.BranchProtection{
			RuleName:        "develop",
			BranchName:      "develop",
			Priority:        1,
			EnablePush:      true,
		...
		changeFilesOptions.Files[1].Path = updateTreePath
		changeFilesOptions.Files[2].Path = deleteTreePath
		req = NewRequestWithJSON(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/contents", user2.Name, repo16.Name), &changeFilesOptions).
			AddTokenAuth(token4)
		...
		changeFilesOptions.Files[1].Path = updateTreePath
		changeFilesOptions.Files[2].Path = deleteTreePath
		req = NewRequestWithJSON(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/contents", org3.Name, repo3.Name), &changeFilesOptions).
			AddTokenAuth(token2)
		MakeRequest(t, req, http.StatusCreated)
