...
func TestAPIIssuesMilestone(t *testing.T) {
	...
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/milestones/%d", owner.Name, repo.Name, milestone.ID)
	...
	req = NewRequestWithJSON(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/milestones", owner.Name, repo.Name), structs.CreateMilestoneOption{
		Title:       "wow",
	...
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/milestones?state=%s", owner.Name, repo.Name, "all")).
		AddTokenAuth(token)
	resp = MakeRequest(t, req, http.StatusOK)
	...
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/milestones/%s", owner.Name, repo.Name, apiMilestones[2].Title)).
		AddTokenAuth(token)
	resp = MakeRequest(t, req, http.StatusOK)
	apiMilestone = DecodeJSON(t, resp, &structs.Milestone{})
	assert.Equal(t, apiMilestones[2], *apiMilestone)

	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/milestones?state=%s&name=%s", owner.Name, repo.Name, "all", "milestone2")).
		AddTokenAuth(token)
	resp = MakeRequest(t, req, http.StatusOK)
	apiMilestones = DecodeJSON(t, resp, []structs.Milestone{})
	...
	req = NewRequest(t, "DELETE", fmt.Sprintf("/v1/repos/%s/%s/milestones/%d", owner.Name, repo.Name, apiMilestone.ID)).
		AddTokenAuth(token)
