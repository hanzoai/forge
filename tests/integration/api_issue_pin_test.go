...
func TestAPIPinIssue(t *testing.T) {
	...
	// Pin the Issue
	req := NewRequest(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/pin", repo.OwnerName, repo.Name, issue.Index)).
		AddTokenAuth(token)
	MakeRequest(t, req, http.StatusNoContent)

	// Check if the Issue is pinned
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/%d", repo.OwnerName, repo.Name, issue.Index))
...
func TestAPIUnpinIssue(t *testing.T) {
	...
	// Unpin the Issue
	req = NewRequest(t, "DELETE", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/pin", repo.OwnerName, repo.Name, issue.Index)).
		AddTokenAuth(token)
	MakeRequest(t, req, http.StatusNoContent)
...
func TestAPIMoveIssuePin(t *testing.T) {
	...
	req = NewRequest(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/pin", repo.OwnerName, repo.Name, issue2.Index)).
		AddTokenAuth(token)
	MakeRequest(t, req, http.StatusNoContent)

	// Move the first Issue to position 2
	req = NewRequest(t, "PATCH", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/pin/2", repo.OwnerName, repo.Name, issue.Index)).
		AddTokenAuth(token)
	MakeRequest(t, req, http.StatusNoContent)
	...
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/%d", repo.OwnerName, repo.Name, issue2.Index))
	resp = MakeRequest(t, req, http.StatusOK)
...
func TestAPIListPinnedIssues(t *testing.T) {
	...
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/pinned", repo.OwnerName, repo.Name))
	resp := MakeRequest(t, req, http.StatusOK)
...
func TestAPIListPinnedPullrequests(t *testing.T) {
	...
	req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/pulls/pinned", repo.OwnerName, repo.Name))
	resp := MakeRequest(t, req, http.StatusOK)
...
func TestAPINewPinAllowed(t *testing.T) {
	...
	req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/new_pin_allowed", owner.Name, repo.Name))
	resp := MakeRequest(t, req, http.StatusOK)
