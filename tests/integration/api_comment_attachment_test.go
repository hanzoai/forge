...
func TestAPIGetCommentAttachment(t *testing.T) {
	...
	t.Run("UnrelatedCommentID", func(t *testing.T) {
		...
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/issues/comments/%d/assets/%d", repoOwner.Name, repo.Name, comment.ID, attachment.ID).
			AddTokenAuth(token)
...
func TestAPIListCommentAttachments(t *testing.T) {
	...
	session := loginUser(t, repoOwner.Name)
	token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeReadIssue)
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/issues/comments/%d/assets", repoOwner.Name, repo.Name, comment.ID).
		AddTokenAuth(token)
	resp := session.MakeRequest(t, req, http.StatusOK)
...
func TestAPICreateCommentAttachment(t *testing.T) {
	...
	req := NewRequestWithBody(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/issues/comments/%d/assets", repoOwner.Name, repo.Name, comment.ID), body).
		AddTokenAuth(token).
...
func TestAPIEditCommentAttachment(t *testing.T) {
	...
	session := loginUser(t, repoOwner.Name)
	token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeWriteIssue)
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/comments/%d/assets/%d",
...
func TestAPIDeleteCommentAttachment(t *testing.T) {
	...
	req := NewRequest(t, "DELETE", fmt.Sprintf("/v1/repos/%s/%s/issues/comments/%d/assets/%d", repoOwner.Name, repo.Name, comment.ID, attachment.ID)).
		AddTokenAuth(token)
