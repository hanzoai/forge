...
func TestAPIManageEmailsFeatureDisabled(t *testing.T) {
	...
	addReq := NewRequestWithJSON(t, "POST", "/v1/user/emails", &api.CreateEmailOption{
		Emails: []string{"user2-3@example.com"},
	}).AddTokenAuth(token)
	MakeRequest(t, addReq, http.StatusNotFound)

	delReq := NewRequestWithJSON(t, "DELETE", "/v1/user/emails", &api.DeleteEmailOption{
...
func TestAPIListEmails(t *testing.T) {
	...
	req := NewRequest(t, "GET", "/v1/user/emails").
		AddTokenAuth(token)
...
func TestAPIAddEmail(t *testing.T) {
	...
	req := NewRequestWithJSON(t, "POST", "/v1/user/emails", &opts).
		AddTokenAuth(token)
...
func TestAPIDeleteEmail(t *testing.T) {
	...
	req := NewRequestWithJSON(t, "DELETE", "/v1/user/emails", &opts).
		AddTokenAuth(token)
