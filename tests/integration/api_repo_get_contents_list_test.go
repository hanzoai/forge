...
func TestAPIGetContentsList(t *testing.T) {
	onGitRun(t, testAPIGetContentsList)
}
...
func testAPIGetContentsList(t *testing.T, u *url.URL) {
	...
	refType := "branch"
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/contents?ref=%s", user2.Name, repo1.Name, ref)
	...
	// No ref
	refType = "branch"
	req = NewRequestf(t, "GET", "/v1/repos/%s/%s/contents/", user2.Name, repo1.Name)
	resp = MakeRequest(t, req, http.StatusOK)
	contentsListResponse = DecodeJSON(t, resp, []*api.ContentsResponse{})
	assert.NotNil(t, contentsListResponse)
	...
	req = NewRequestf(t, "GET", "/v1/repos/%s/%s/contents/", user2.Name, repo16.Name).
		AddTokenAuth(token4)
	MakeRequest(t, req, http.StatusNotFound)
	...
	req = NewRequestf(t, "GET", "/v1/repos/%s/%s/contents/", org3.Name, repo3.Name).
		AddTokenAuth(token2)
	MakeRequest(t, req, http.StatusOK)
}
