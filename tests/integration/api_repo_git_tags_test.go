...
func TestAPIGitTags(t *testing.T) {
	...
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/git/tags/%s", user.Name, repo.Name, aTag.ID.String()).
		AddTokenAuth(token)
	...
	badReq := NewRequestf(t, "GET", "/v1/repos/%s/%s/git/tags/%s", user.Name, repo.Name, commit.ID.String()).
		AddTokenAuth(token)
...
func TestAPIDeleteTagByName(t *testing.T) {
	...
	req := NewRequest(t, http.MethodDelete, fmt.Sprintf("/v1/repos/%s/%s/tags/delete-tag", owner.Name, repo.Name)).
		AddTokenAuth(token)
	...
	req = NewRequest(t, http.MethodDelete, fmt.Sprintf("/v1/repos/%s/%s/tags/release-tag", owner.Name, repo.Name)).
		AddTokenAuth(token)
