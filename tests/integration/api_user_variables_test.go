...
func TestAPIUserVariables(t *testing.T) {
	...
	t.Run("CreateUserVariable", func(t *testing.T) {
		...
		for _, c := range cases {
			req := NewRequestWithJSON(t, "POST", "/v1/user/actions/variables/"+c.Name, api.CreateVariableOption{
				Value: "value",
			}).AddTokenAuth(token)
	...
	t.Run("UpdateUserVariable", func(t *testing.T) {
		...
		url := "/v1/user/actions/variables/" + variableName
		...
		for _, c := range cases {
			req := NewRequestWithJSON(t, "PUT", "/v1/user/actions/variables/"+c.Name, api.UpdateVariableOption{
				Name:  c.UpdateName,
				Value: "updated_val",
			}).AddTokenAuth(token)
			MakeRequest(t, req, c.ExpectedStatus)
