...
func TestAPIGetIssueAttachment(t *testing.T) {
	...
	req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets/%d", repoOwner.Name, repo.Name, issue.Index, attachment.ID)).
		AddTokenAuth(token)
...
func TestAPIListIssueAttachments(t *testing.T) {
	...
	req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets", repoOwner.Name, repo.Name, issue.Index)).
		AddTokenAuth(token)
	resp := session.MakeRequest(t, req, http.StatusOK)
	apiAttachment := DecodeJSON(t, resp, []api.Attachment{})
...
func TestAPICreateIssueAttachment(t *testing.T) {
	...
	req := NewRequestWithBody(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets", repoOwner.Name, repo.Name, issue.Index), body).
		AddTokenAuth(token)
...
func TestAPIEditIssueAttachment(t *testing.T) {
	...
	session := loginUser(t, repoOwner.Name)
	token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeWriteIssue)
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets/%d",
...
func TestAPIEditIssueAttachmentWithUnallowedFile(t *testing.T) {
	...
	filename := "file.bad"
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets/%d", repoOwner.Name, repo.Name, issue.Index, attachment.ID)
	req := NewRequestWithValues(t, "PATCH", urlStr, map[string]string{
		"name": filename,
	}).AddTokenAuth(token)
...
func TestAPIDeleteIssueAttachment(t *testing.T) {
	...
	req := NewRequest(t, "DELETE", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/assets/%d", repoOwner.Name, repo.Name, issue.Index, attachment.ID)).
		AddTokenAuth(token)
