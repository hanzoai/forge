...
func TestAPIListRepoComments(t *testing.T) {
	...
	link, _ := url.Parse(fmt.Sprintf("/v1/repos/%s/%s/issues/comments", repoOwner.Name, repo.Name))
...
func TestAPIListIssueComments(t *testing.T) {
	...
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/issues/%d/comments", repoOwner.Name, repo.Name, issue.Index).
		AddTokenAuth(token)
	resp := MakeRequest(t, req, http.StatusOK)
...
func TestAPICreateComment(t *testing.T) {
	...
	urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/%d/comments",
	...
	t.Run("BlockedByRepoOwner", func(t *testing.T) {
		...
		req := NewRequestWithValues(t, "POST", fmt.Sprintf("/v1/repos/%s/%s/issues/%d/comments", repo.OwnerName, repo.Name, issue.Index), map[string]string{
			"body": commentBody,
...
func TestAPIGetComment(t *testing.T) {
	...
	token := getUserToken(t, repoOwner.Name, auth_model.AccessTokenScopeReadIssue)
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/issues/comments/%d", repoOwner.Name, repo.Name, comment.ID)
...
func TestAPIEditComment(t *testing.T) {
	...
	t.Run("UnrelatedCommentID", func(t *testing.T) {
		...
		repoOwner := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: repo.OwnerID})
		token := getUserToken(t, repoOwner.Name, auth_model.AccessTokenScopeWriteIssue)
		urlStr := fmt.Sprintf("/v1/repos/%s/%s/issues/comments/%d",
...
func TestAPIDeleteComment(t *testing.T) {
	...
	t.Run("UnrelatedCommentID", func(t *testing.T) {
		...
		repoOwner := unittest.AssertExistsAndLoadBean(t, &user_model.User{ID: repo.OwnerID})
		token := getUserToken(t, repoOwner.Name, auth_model.AccessTokenScopeWriteIssue)
		req := NewRequestf(t, "DELETE", "/v1/repos/%s/%s/issues/comments/%d", repoOwner.Name, repo.Name, comment.ID).
			AddTokenAuth(token)
		MakeRequest(t, req, http.StatusNotFound)
	})
...
func TestAPIListIssueTimeline(t *testing.T) {
	...
	// make request
	req := NewRequestf(t, "GET", "/v1/repos/%s/%s/issues/%d/timeline", repoOwner.Name, repo.Name, issue.Index)
	resp := MakeRequest(t, req, http.StatusOK)
