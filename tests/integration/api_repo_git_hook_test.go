...
`
	...
	t.Run("ListGitHooks", func(t *testing.T) {
		...
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/hooks/git", owner.Name, repo.Name).
			AddTokenAuth(token)
	...
	t.Run("GetGitHook", func(t *testing.T) {
		...
		session := loginUser(t, "user1")
		token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeReadRepository)
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/hooks/git/pre-receive", owner.Name, repo.Name).
			AddTokenAuth(token)
		resp := MakeRequest(t, req, http.StatusOK)
	...
	t.Run("EditGitHook", func(t *testing.T) {
		...
		urlStr := fmt.Sprintf("/v1/repos/%s/%s/hooks/git/pre-receive",
	...
	t.Run("EditGitHookNoAccess", func(t *testing.T) {
		...
		session := loginUser(t, owner.Name)
		token := getTokenForLoggedInUser(t, session, auth_model.AccessTokenScopeWriteRepository)
		urlStr := fmt.Sprintf("/v1/repos/%s/%s/hooks/git/pre-receive", owner.Name, repo.Name)
		req := NewRequestWithJSON(t, "PATCH", urlStr, &api.EditGitHookOption{
			Content: testHookContent,
		}).AddTokenAuth(token)
		MakeRequest(t, req, http.StatusForbidden)
	...
	t.Run("DeleteGitHook", func(t *testing.T) {
		...
		req := NewRequestf(t, "DELETE", "/v1/repos/%s/%s/hooks/git/pre-receive", owner.Name, repo.Name).
			AddTokenAuth(token)
