...
func TestAPIRepoCollaboratorPermission(t *testing.T) {
	...
	t.Run("RepoOwnerShouldBeOwner", func(t *testing.T) {
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, repo2Owner.Name).
	...
	t.Run("CollaboratorWithReadAccess", func(t *testing.T) {
		...
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, user4.Name).
			AddTokenAuth(testCtx.Token)
		resp := MakeRequest(t, req, http.StatusOK)
	...
	t.Run("CollaboratorNotFound", func(t *testing.T) {
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, "non-existent-user").
			AddTokenAuth(testCtx.Token)
	...
	t.Run("CollaboratorCanQueryItsPermissions", func(t *testing.T) {
		...
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, user5.Name).
		...
		t.Run("CollaboratorCanReadOwnPermission", func(t *testing.T) {
			...
			req = NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, user5.Name).AddTokenAuth(token)
			resp = MakeRequest(t, req, http.StatusOK)
	...
	t.Run("RepoAdminCanQueryACollaboratorsPermissions", func(t *testing.T) {
		...
		req := NewRequestf(t, "GET", "/v1/repos/%s/%s/collaborators/%s/permission", repo2Owner.Name, repo2.Name, user11.Name).
			AddTokenAuth(_testCtx.Token)
		resp := _session.MakeRequest(t, req, http.StatusOK)
