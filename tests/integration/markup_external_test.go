	...
	t.Run("RenderContentInIFrame", func(t *testing.T) {
		t.Run("DefaultSandbox", func(t *testing.T) {
			...
			t.Run("FramePage", func(t *testing.T) {
				...
				assert.Equal(t,
					`<script nonce crossorigin src="`+public.AssetURI("web_src/js/external-render-helper.ts")+`" id="hanzo-external-render-helper" data-render-query-string=""></script>`+
		...
		t.Run("NoSanitizerNoSandbox", func(t *testing.T) {
			...
			t.Run("HTMLContentWithExternalRenderIframeHelper", func(t *testing.T) {
				...
				respSub := MakeRequest(t, req, http.StatusOK)
				assert.Equal(t,
					`<script nonce crossorigin src="`+public.AssetURI("web_src/js/external-render-helper.ts")+`" id="hanzo-external-render-helper" data-render-query-string="a=1%2f2"></script>`+
						`<link rel="stylesheet" href="`+public.AssetURI("web_src/css/themes/theme-hanzo-auto.css")+`">`+
