...
func TestAPITopicSearch(t *testing.T) {
	...
	searchURL, _ := url.Parse("/v1/topics/search")
...
func TestAPIRepoTopic(t *testing.T) {
	...
	req := NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/topics", user2.Name, repo2.Name)).
		AddTokenAuth(token2)
	...
	// Test delete a topic
	req = NewRequestf(t, "DELETE", "/v1/repos/%s/%s/topics/%s", user2.Name, repo2.Name, "Topicname1").
		AddTokenAuth(token2)
	MakeRequest(t, req, http.StatusNoContent)

	// Test add an existing topic
	req = NewRequestf(t, "PUT", "/v1/repos/%s/%s/topics/%s", user2.Name, repo2.Name, "Golang").
		AddTokenAuth(token2)
	MakeRequest(t, req, http.StatusNoContent)

	// Test add a topic
	req = NewRequestf(t, "PUT", "/v1/repos/%s/%s/topics/%s", user2.Name, repo2.Name, "topicName3").
		AddTokenAuth(token2)
	MakeRequest(t, req, http.StatusNoContent)

	url := fmt.Sprintf("/v1/repos/%s/%s/topics", user2.Name, repo2.Name)
	...
	req = NewRequestf(t, "PUT", "/v1/repos/%s/%s/topics/%s", user2.Name, repo2.Name, "t26").
		AddTokenAuth(token2)
	MakeRequest(t, req, http.StatusUnprocessableEntity)
	...
	req = NewRequest(t, "GET", fmt.Sprintf("/v1/repos/%s/%s/topics", org3.Name, repo3.Name)).
		AddTokenAuth(token4)
	res = MakeRequest(t, req, http.StatusOK)
	topics = DecodeJSON(t, res, &api.TopicName{})
	...
	req = NewRequestf(t, "PUT", "/v1/repos/%s/%s/topics/%s", org3.Name, repo3.Name, "topicName").
		AddTokenAuth(token4)
